  ...
  describe('detects infinite setImmediate loop when there are enough calls', function() {
    it('throws on infinite loop detection - for a global timer', function() {
      return zurvan
        .interceptTimers()
        .then(function() {
          var onlyOneSuchFunctionInWholeProgram = function() {
        ...
        .then(
          ...
          function(err) {
            ...
            assert(
              new RegExp('Possible infinite setImmediate loop detected').test(
                err.message
              )
            );
        ...
        .then(function(res) {
          ...
          Object.keys(res.immediates).forEach(function(k) {
            if (k !== 'size') {
              assert(
                new RegExp('onlyOneSuchFunctionInWholeProgram').test(
                  res.immediates[k]
                )
              );
            }
