...
var bluebirdCompatibilityTestcase = function(
  configuration,
  expectedOutput,
  bluebird
) {
...
describe('zurvan', function() {
  ...
  describe('under special configuration (for compatibility with external libraries)', function() {
    it(
      'specially for bluebird changes its scheduler',
      bluebirdCompatibilityTestcase({ bluebird: bluebird }, 1, bluebird)
    );
    it('throws on wrong type of bluebird configuration parameter', function(done) {
      zurvan.interceptTimers({ bluebird: {} }).then(
        function() {
