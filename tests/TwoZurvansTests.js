...
var zurvan = require('../zurvan').withDefaultConfiguration({
  systemTime: '2015-09-01T00:00:00.000Z'
});
...
describe('second zurvan', function() {
  it('shall reject intercepting timers if they are already intercepted', function() {
    ...
    return zurvan
      ...
      .then(
        function() {
          throw new Error(
            'Zurvan should not be able to intercept timers that are already intercepted'
          );
        },
        function(err) {
          assert(
            err.message.match(
              /Cannot intercept timers that are already intercepted/
            )
          );
        }
      )
  ...
  it('cannot forward time if timers were intercepted by the other instance', function() {
    ...
    return zurvan2
      ...
      .then(
        function(err) {
          throw new Error(
            'Zurvan should not be able to advance time before intercepting it'
          );
        },
        function(err) {
          ...
          assert(
            err.message.match(
              /Cannot advance time if timers are not intercepted by this instance/
            )
          );
    ...
    zurvan
      ...
      .then(
        function() {
            ...
            new Error('Should not release timers that were not intercepted')
          );
        },
        function() {
  ...
  it('after configuration reset only new instance has default configuration (old configuration is not modified)', function() {
    var zurvanForwardedStart = zurvan.withDefaultConfiguration({
      timeSinceStartup: 100
    });
    var zurvanDefault = zurvanForwardedStart.withDefaultConfiguration();

    return zurvanForwardedStart
      .interceptTimers()
      .then(function() {
      ...
      .then(
        function() {
          return zurvanForwardedStart.releaseTimers().then(Promise.reject);
        },
        function(err) {
          return zurvanForwardedStart.releaseTimers();
        }
