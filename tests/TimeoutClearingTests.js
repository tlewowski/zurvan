  ...
  describe('after intercepting timers', function() {
    ...
    it('creates unstrigifiable handles', function(done) {
      ...
      assert.throws(function() {
        JSON.stringify(immediateHandle);
      });
      assert.throws(function() {
        JSON.stringify(timeoutHandle);
      });
      assert.throws(function() {
        JSON.stringify(intervalHandle);
      });
      done();
    });
  });
    ...
    it('rejects if timers were not intercepted', function(done) {
      zurvan.releaseTimers().then(
        function() {
            ...
            new Error(
              'Timers were not intercepted yet - releasing shall be rejected'
            )
          );
        },
        function() {
      ...
      zurvan
        ...
        .then(
          function() {
              ...
              new Error(
                'Timers were already intercepted - shall not be able to intercept them again'
              )
            );
          },
          function() {
