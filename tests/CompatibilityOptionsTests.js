...
describe('zurvan', function() {
  ...
  describe('under special configuration (for compatibility)', function() {
    ...
    beforeEach(function() {
      timeoutCalledOverrider = new FieldOverrider(
        global,
        'timeoutCalled',
        false
      );
    ...
    it('does not intercept process timers (browsers do not have it)', function() {
      return zurvan
        .interceptTimers({ ignoreProcessTimers: true })
        .then(function() {
    ...
    it('evaluates timer or interval set with string as first argument (nodejs does not)', function() {
      return zurvan
        .interceptTimers({ acceptEvalTimers: true })
        .then(function() {
          /*jshint -W066 */
          setTimeout('global.timeoutCalled = true;', 75);
          ...
          /*jshint -W066 */
          setInterval(
            'global.intervalCalled = global.intervalCalled || 0; ++global.intervalCalled;',
            50
          );
    ...
    it('denies implicit timeout time (by default set to 1 millisecond) - it is a mighty bad habit, possibly an error', function() {
      var called;
      return zurvan
        .interceptTimers({ denyImplicitTimer: true })
        .then(function() {
          setTimeout(
            function() {
              called = true;
        ...
        .then(
          function() {
            throw new Error(
              'Implicit timer shall be denied and setting timer shall throw'
            );
          },
          function() {
            return zurvan.releaseTimers();
    ...
    it('denies negative timeout time (by default set to 1 millisecond) - it is a mighty bad habit, possibly an error', function() {
      var called;
      return zurvan
        .interceptTimers({ denyTimersShorterThan1Ms: true })
        .then(function() {
          setTimeout(function() {
            called = true;
          }, -1);
        ...
        .then(
          function() {
            throw new Error(
              'Negative timer shall be denied and setting timer shall throw'
            );
          },
          function() {
            return zurvan.releaseTimers();
    ...
    it('denies time over Int32 (by default set to 1 millisecond) - it is likely an error', function() {
      var called;
      return zurvan
        .interceptTimers({ denyTimersLongerThanInt32: true })
        .then(function() {
          setTimeout(function() {
            called = true;
          }, Math.pow(2, 33));
        ...
        .then(
          function() {
            throw new Error(
              'Too long timer shall be denied and setting timer shall throw'
            );
          },
          function() {
            return zurvan.releaseTimers();
