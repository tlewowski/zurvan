...
function AllTimersInterceptor(timeServer) {
  ...
  this._timeoutInterceptor = new TimerInterceptor(
    timeServer,
    TimerTypes.timeout
  );
  this._intervalInterceptor = new TimerInterceptor(
    timeServer,
    TimerTypes.interval
  );
}
...
AllTimersInterceptor.prototype.intercept = function(config) {
  ...
  this._timerExpirationPolicy =
    TimerExpirationPolicies[config.timerExpirationPolicy];
};
...
AllTimersInterceptor.prototype._nextTimersGroup = function(
  nextTimeouts,
  nextIntervals
) {
  if (!nextTimeouts[0]) {
...
AllTimersInterceptor.prototype.nextTimer = function() {
  ...
  return this._timerExpirationPolicy.selectElement(
    this._nextTimersGroup(nextTimeouts, nextIntervals)
  );
};
