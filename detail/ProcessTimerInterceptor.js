...
ProcessTimerInterceptor.prototype.intercept = function() {
  this.uptimeOverrider = new FieldOverrider(
    process,
    'uptime',
    this.uptime.bind(this)
  );
  this.hrtimeOverrider = new FieldOverrider(
    process,
    'hrtime',
    this.hrtime.bind(this)
  );
};
...
function toHrtimeFormat(time) {
  return [
    Math.floor(time.toSeconds()),
    time.toNanoseconds() %
      (TimeUnit.seconds.coefficient / TimeUnit.nanoseconds.coefficient)
  ];
}
...
ProcessTimerInterceptor.prototype.hrtime = function(previousValue) {
  if (previousValue !== undefined) {
    ...
    var previousTime = TimeUnit.seconds(previousValue[0]).extended(
      TimeUnit.nanoseconds(previousValue[1])
    );
