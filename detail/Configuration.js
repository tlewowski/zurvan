...
function requestBoolean(name) {
  return function(config) {
    if (!TypeChecks.isBoolean(config[name])) {
      return (
        'required ' + name + ' to be a boolean (given: ' + config[name] + ')'
      );
    }
  };
}
  ...
  return function(config) {
    if (!TypeChecks.isInteger(config[name]) || config[name] <= 0) {
      return (
        'required ' +
        name +
        ' to be a positive integer (given: ' +
        config[name] +
        ')'
      );
    }
...
var validators = [
  function negativeTimeSinceStartup(config) {
    if (
      ...
      return (
        'if timeSinceStartup is a number, it has to be >= 0 (given: ' +
        config.timeSinceStartup +
        ')'
      );
    }
    ...
    if (
      ...
      return (
        'timeSinceStartup has to be either a non-negative number, instance of TimeUnit or ' +
        'an array of two positive numbers (given: ' +
        config.timeSinceStartup +
        ')'
      );
    }
    ...
    if (Array.isArray(config.timeSinceStartup)) {
      if (config.timeSinceStartup.length != 2) {
        return (
          'timeSinceStartup as array must contain exactly two elements (given length: ' +
          config.timeSinceStartup.length +
          ')'
        );
      }
      if (
        config.timeSinceStartup.filter(function(x) {
          return !TypeChecks.isNumber(x) || x < 0;
        }).length > 0
      ) {
        return (
          'timeSinceStartup as array must contain two positive numbers (given: ' +
          config.timeSinceStartup +
          ')'
        );
      }
  ...
  function(config) {
    if (isNaN(new Date(config.systemTime).getTime())) {
      return (
        'systemTime must be an argument of new Date resulting in valid date (given: ' +
        config.systemTime +
        ')'
      );
    }
