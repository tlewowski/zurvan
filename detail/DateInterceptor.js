...
function fakeDate(timeServer) {
  ...
  function makeOriginalDateFromArgs(
    ...
    if (month === undefined) {
      // means: first argument is a timestamp
      return new OriginalDate(yearOrTimeAsStringOrTimestamp);
    }
  ...
  FakeDate.now = function() {
    return Math.floor(
      timeServer.systemTimeOffset
        .extended(timeServer.currentTime)
        .toMilliseconds()
    );
  };
...
DateInterceptor.prototype.intercept = function() {
  this.dateOverrider = new FieldOverrider(
    global,
    'Date',
    this._fakeDateCreator
  );
};
